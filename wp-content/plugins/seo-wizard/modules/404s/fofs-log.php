...
function WSW_fofs_log_export_filter($all_settings) {
	unset($all_settings['404s']['log']);
	return $all_settings;
}
add_filter('WSW_settings_export_array', 'WSW_fofs_log_export_filter');

class WSW_FofsLog extends SEO_Module {
	...
	static function get_module_title() { return __('404 Monitor Log', 'seo-wizard'); }
	function get_module_subtitle() { return __('Log', 'seo-wizard'); }
	function get_default_status() { return WSW_MODULE_DISABLED; }
	...
	function init() {
		...
		add_action('WSW_save_hit', array(&$this, 'log_hit'));
	}
	...
	function get_admin_table_columns() {
		return array(
			...
			, 'hit-count' => __('Hits', 'seo-wizard')
			, 'url' => __('URL with 404 Error', 'seo-wizard')
			, 'last-hit-time' => __('Date of Most Recent Hit', 'seo-wizard')
			, 'referers' => __('Referers', 'seo-wizard')
			, 'user-agents' => __('User Agents', 'seo-wizard')
		);
	}
	...
	function admin_page_contents() {
		...
		if (!$this->get_setting('log_enabled', true))
			$this->queue_message('warning', __('New 404 errors will not be recorded because 404 logging is disabled on the Settings tab.', 'seo-wizard'));
		...
		if ($this->is_action('delete')) {
			...
			if (isset($the404s[$_GET['object']])) {
				...
				$this->queue_message('success', __('The log entry was successfully deleted.', 'seo-wizard'));
			} else
				$this->queue_message('error', __('This log entry has already been deleted.', 'seo-wizard'));
		...
		//Are we clearing the whole 404 log?
			...
			$this->queue_message('success', __('The log was successfully cleared.', 'seo-wizard'));
		}
		
		if (!count($the404s))
			$this->queue_message('success', __('No 404 errors in the log.', 'seo-wizard'));
		...
		if (count($the404s)) {
			...
			foreach ($the404s as $url => $data) {
				...
				$ae_url = WSW_esc_attr(urlencode($url));
				$md5url = md5($url);
				...
				$this->table_cells(array(
					  'actions' =>
							  "<span class='su-404s-hit-open'><a href='$a_url' target='_blank'><img src='{$this->module_dir_url}hit-open.png' title='".__('Open URL in new window (will not be logged)', 'seo-wizard')."' /></a></span>"
							. "<span class='su-404s-hit-cache'><a href='http://www.google.com/search?q=cache%3A{$ae_url}' target='_blank'><img src='{$this->module_dir_url}hit-cache.png' title='".__('Query Google for cached version of URL (opens in new window)', 'seo-wizard')."' /></a></span>"
							. "<span class='su-404s-hit-delete'><a href='".$this->get_nonce_url('delete', $url)."'><img src='{$this->module_dir_url}hit-delete.png' title='".__('Remove this URL from the log', 'seo-wizard')."' /></a></span>"
					, 'hit-count' => $data['hit_count']
					, 'url' => "<attr title='$a_url'>" . esc_html(sustr::truncate($url, 100)) . '</attr>'
					, 'last-hit-time' => sprintf(__('%s at %s', 'seo-wizard')
						, date_i18n(get_option('date_format'), $data['last_hit_time'])
						, date_i18n(get_option('time_format'), $data['last_hit_time'])
						)
					, 'referers' => number_format_i18n(count($data['referers'])) . (count($data['referers']) ? " <a href='#' class='WSW_toggle_hide' data-toggle='su-404s-hit-$md5url-referers'><img src='{$this->module_dir_url}hit-details.png' title='".__('View list of referring URLs', 'seo-wizard')."' /></a>" : '')
					, 'user-agents' => number_format_i18n(count($data['user_agents'])) . (count($data['user_agents']) ? " <a href='#' class='WSW_toggle_hide' data-toggle='su-404s-hit-$md5url-user-agents'><img src='{$this->module_dir_url}hit-details.png' title='".__('View list of user agents', 'seo-wizard')."' /></a>" : '')
				));
				...
				if (count($data['referers'])) {
					...
					echo "\t\t\t<div><strong>".__('Referring URLs', 'seo-wizard')."</strong> &mdash; ";
					echo "<a href='#' class='WSW_toggle_up' data-toggle='su-404s-hit-$md5url-referers'>".__('Hide list', 'seo-wizard')."</a>";
					echo "</div>\n";
					echo "\t\t\t<ul>\n";
					
					foreach ($data['referers'] as $referer) {
						$referer = WSW_esc_attr($referer); //Don't let attacks pass through the referer URLs!
				...
				if (count($data['user_agents'])) {
					...
					echo "\t\t\t<div><strong>".__('User Agents', 'seo-wizard')."</strong> &mdash; ";
					echo "<a href='#' class='WSW_toggle_up' data-toggle='su-404s-hit-$md5url-user-agents'>".__('Hide list', 'seo-wizard')."</a>";
					echo "</div>\n";
					echo "\t\t\t<ul>\n";
					
					foreach ($data['user_agents'] as $useragent) {
						$useragent = WSW_esc_html($useragent); //Don't let attacks pass through the user agent strings!
