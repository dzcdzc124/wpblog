...
'use strict';
(function( $ ) {
	$.fn.wsw_redirects = function( arg_type ) {
		var that = this;
		...
		this.edit_row = function( row, cancellable ) {
			...
			$.each( $( row ).find( '.val' ), function( k, v ) {
				...
				if ( $( v ).hasClass( 'type' ) ) {
					...
					$.each( $( that ).find( '#wsw_redirects_new_type option' ), function( k, v ) {
		...
		this.delete_row = function( row ) {
			$( row ).fadeTo( 'fast', 0 ).slideUp( function() {
				...
				$.post(
					ajaxurl,
					{
						action: 'wsw_delete_redirect_' + type,
		...
		this.save_redirect = function( row ) {
			...
			// Check old URL
			if ( '' === old_url ) {
				window.alert( "The old url can't empty. Please insert the old url." );
				return false;
			}

			// Check new URL
			if ( '' === new_url && '410' !== redirect_type ) {
				window.alert( "The new url can't empty. Please insert the new url.");
				return false;
			}

			// GCheck the redirect type
			if ( '' === redirect_type ) {
                window.alert( "The new type can't empty.");
				return false;
			}
			...
			$.post(
				ajaxurl,
				{
					action: 'wsw_save_redirect_' + type,
					ajax_nonce: $( '.wsw_redirects_ajax_nonce' ).val(),
		...
		this.add_redirect = function( old_redirect, new_redirect, redirect_type ) {
			if ( '' === old_redirect ) {
				if ( 'url' === type ) {
                    ...
                    window.alert( "The old regular url can't empty.");
				}
			...
			if ( new_redirect === old_redirect ) {
                window.alert("You can't redirect a URL to itself");
			...
			// Empty fields
			that.find( '#wsw_redirects_old' ).val( '' );
			that.find( '#wsw_redirects_new' ).val( '' );
			...
			$.post(
				ajaxurl,
				{
					action: 'wsw_create_redirect_' + type,
					ajax_nonce: $( '.wsw_redirects_ajax_nonce' ).val(),
			...
			$.post(
				...
				function( response ) {
					...
					if ( '200' !== response_obj.reponse_code ) {
                        window.alert( wsw_redirects_strings.redirect_possibly_bad, wsw_redirects_strings.redirect_not_ok + ( ( response_obj.reponse_code !== '' ) ? '<br/><br/><b>HTTP Code: ' + response_obj.reponse_code + '</b>' : '' ) );
					}
				}
			);
		};
		...
		this.setup = function() {
			...
			that.find( '.wsw-new-redirect-form div a.btn-primary' ).click( function() {
				that.add_redirect( that.find( '#wsw_redirects_old' ).val(), that.find( '#wsw_redirects_new' ).val(), that.find( '#wsw_redirects_new_type' ).val() );
				return false;
			}
			);

			that.find( '.wsw-new-redirect-form div input' ).keypress( function( event ) {
				if ( event.which === 13 ) {
					event.preventDefault();
					that.add_redirect( that.find( '#wsw_redirects_old' ).val(), that.find( '#wsw_redirects_new_' ).val(), that.find( '#wsw_redirects_new_type' ).val() );
				}
			}
			);
			...
			$( window ).on( 'beforeunload', function() {
				if ( $( '.row_edit' ).length > 0 ) {
					return wsw_redirects_strings.unsaved_redirects;
				}
			}
			);
		};
