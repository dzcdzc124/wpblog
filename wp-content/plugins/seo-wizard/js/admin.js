...
/*************** for dynamic front page **********************/
  jQuery(document).ready(function(){
     if(!jQuery('#chk_homepage_static').is(':checked'))
     {
         jQuery('#home-dynamic-settings').show();
     }
      jQuery('#chk_homepage_static').on('change', function(){
          if(jQuery(this).is(':checked'))
          {
              jQuery('#home-dynamic-settings').hide('slow');
          }
          else
          {
              jQuery('#home-dynamic-settings').show('slow');
          }
      });
      var count = 0;
      var remaining = 0;

      if(document.getElementById("wsw_homepage_title") !== null)
      {
          var home_title = jQuery('#wsw_homepage_title');
          remaining = home_title.val().length > 0 ? (60 - home_title.val().length) : '';
          jQuery('#title_left_numbers').val(remaining);
      }

      if(document.getElementById("wsw_homepage_desc") !== null)
  {
      var home_desc;
      home_desc = jQuery('#wsw_homepage_desc');
      remaining = home_desc.val().length > 0 ? (160 - home_desc.val().length) : '';
      jQuery('#desc_left_numbers').val(remaining);
  }

      jQuery('#wsw_homepage_title').keyup(function(){
          count = jQuery(this).val().length;
         remaining = 60-count;
          jQuery('#title_left_numbers').val(remaining);
      });
      jQuery('#wsw_homepage_desc').keyup(function(){
          count = jQuery(this).val().length;
          remaining = 160 - count;
          jQuery('#desc_left_numbers').val(remaining);
      });
...
function save_global_settings() {
    ...
    var data = {
        ...
        chk_homepage_static: jQuery('#chk_homepage_static').attr('checked') ? "1" : "0",
        wsw_homepage_title: jQuery('#chk_homepage_static').attr('checked') ? '' : jQuery('#wsw_homepage_title').val() ,
        wsw_homepage_desc: jQuery('#chk_homepage_static').attr('checked') ? '' : jQuery('#wsw_homepage_desc').val() ,
        wsw_homepage_keywords: jQuery('#chk_homepage_static').attr('checked') ? '' : jQuery('#wsw_homepage_keywords').val()
        /***************************************************************/
    };
